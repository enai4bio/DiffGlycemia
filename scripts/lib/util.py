...
def get_tabnet_config(real_data_path, is_cv=False):
    ds_name = Path(real_data_path).name
    C = load_json(f'tuned_models/tabnet/{ds_name}_cv.json')
    return C
...
def convert_numpy_to_native(config_origin):
    config = deepcopy(config_origin)
    if isinstance(config, dict):
        return {k: convert_numpy_to_native(v) for k, v in config.items()}
    elif isinstance(config, list):
        return [convert_numpy_to_native(item) for item in config]
...
def transfer_to_gputensor(df, device):
    numpy_array = df.to_numpy()
    tensor_from_df = torch.from_numpy(numpy_array)
    tensor_from_df = tensor_from_df.to(device)
