...
class MetricsReport_pr:
    def __init__(self, df_metrics_both):
        self._res = {k: {} for k in ['train','val','test']}
        self._metrics_names = ["precision", "recal", "roc_auc", "pr_auc"]

        self._res['train']["precision"] = np.nan
        self._res['train']["recal"] = np.nan
        self._res['train']["roc_auc"] = np.nan
        self._res['train']["pr_auc"] = np.nan

        self._res['val']["precision"] = df_metrics_both.loc[df_metrics_both['model'].apply(lambda x: 'real' in x), "precision"][0]
        self._res['val']["recall"] = df_metrics_both.loc[df_metrics_both['model'].apply(lambda x: 'real' in x), "recall"][0]
        self._res['val']["roc_auc"] = df_metrics_both.loc[df_metrics_both['model'].apply(lambda x: 'real' in x), "roc_auc"][0] 
        self._res['val']["pr_auc"] = df_metrics_both.loc[df_metrics_both['model'].apply(lambda x: 'real' in x), "pr_auc"][0]

        self._res['test']["precision"] = df_metrics_both.loc[df_metrics_both['model'].apply(lambda x: 'fake' in x), "precision"].iloc[0] 
        self._res['test']["recall"] = df_metrics_both.loc[df_metrics_both['model'].apply(lambda x: 'fake' in x), "recall"].iloc[0]
        self._res['test']["roc_auc"] = df_metrics_both.loc[df_metrics_both['model'].apply(lambda x: 'fake' in x), "roc_auc"].iloc[0] 
        self._res['test']["pr_auc"] = df_metrics_both.loc[df_metrics_both['model'].apply(lambda x: 'fake' in x), "pr_auc"].iloc[0]
    ...
    def get_val_score(self) -> float:
        return self._res["val"]["r2"] if "r2" in self._res["val"] else self._res["val"]["recall"]
    
    def get_test_score(self) -> float:
        return self._res["test"]["r2"] if "r2" in self._res["test"] else self._res["test"]["recall"]
