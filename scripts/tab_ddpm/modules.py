...
def timestep_embedding(timesteps, dim, max_period=10000):
    ...
    """
    half = dim // 2 # 一半cosine， 一半sine，所以有  half = dim // 2
    freqs = torch.exp(
        -math.log(max_period) * torch.arange(start=0, end=half, dtype=torch.float32) / half
    ).to(device=timesteps.device)
    args = timesteps[:, None].float() * freqs[None] # 转换了dim: torch.Size([2304]) -> torch.Size([2304, 1]); [2304, 1] x [1, 64] = [2304, 64]，即每个数据每个特征所增加的噪音，乘法的原因是：每个数据的所有特征所采用的timesteps是一致的，每个特征所采取的freq是一致，从而确定每个cell的噪音
    embedding = torch.cat([torch.cos(args), torch.sin(args)], dim=-1) # 一半cosine， 一半sine，所以有上述Line 30的 half = dim // 2
    if dim % 2: # 如果是单数，要补充， 补充一列全是0的embedding
...
class MLPDiffusion(nn.Module):
    def __init__(self, raw_config):
        ...
        dim_in = raw_config['train_mlp_params']['dim_in']
        self.dim_t = dim_t = raw_config['train_mlp_params']['dim_t']
