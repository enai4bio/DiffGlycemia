...
def extract(a, t, x_shape):
    b, *_ = t.shape # batch_size 
    t = t.to(a.device) # 这批batch数据每个对应的timesteps; 
    out = a.gather(-1, t) # 从a中取出各个timesteps对应的alpha
    while len(out.shape) < len(x_shape):
        out = out[..., None] # 一维变成二维：使得：out.size() 从torch.Size([batch_size]) 成为 torch.Size([batch_size, 1])
    return out.expand(x_shape) # 重复列；使得： out.expand(x_shape).size() 成为 torch.Size([batch_size, 18])
...
def index_to_log_onehot(x, num_classes): # 把category特征转换成one-hot，再转换成 取log
    onehots = []
    for i in range(len(num_classes)):
        onehots.append(F.one_hot(x[:, i], num_classes[i])) # 把category特征转换成one-hot
 
    x_onehot = torch.cat(onehots, dim=1) # 把转换成的one-hot 合并起来
    log_onehot = torch.log(x_onehot.float().clamp(min=1e-30)) # 转换成 取log
    return log_onehot
